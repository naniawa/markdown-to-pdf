...
def generate_diagram_images(plantuml_matches):
    ...
    for i, plantuml_code in enumerate(plantuml_matches, start=1):
        ...
        with open(f"diagram_{i}.puml", "w", encoding="utf-8") as file:
            file.write(plantuml_code)

        #subprocess.run(["plantuml", f"-tpng", f"diagram_{i}.puml"]) #macOS
        #subprocess.run(["plantuml", f"-tsvg", f"images/diagram_{i}.puml"]) #macOS
        subprocess.run(["java","-Dfile.encoding=UTF-8", "-jar", "plantuml.jar","-tsvg", f"diagram_{i}.puml"]) #windowsOS

        #image_paths.append(f"diagram_{i}.png")
        image_paths.append(f"./diagram_{i}.svg")
        #image_paths.append(os.path.abspath(f"diagram_{i}.svg"))
        # 不要なファイルの削除
        #os.remove(f"diagram_{i}.puml")

    return image_paths
"""
def generate_diagram_images(plantuml_matches, output_directory):
    image_paths = []

    for i, plantuml_code in enumerate(plantuml_matches, start=1):
        # PlantUMLを画像に変換
        os.makedirs(output_directory, exist_ok=True)
        output_path = os.path.abspath(os.path.join(output_directory, f"diagram_{i}.puml"))
        with open(output_path, "w", encoding="utf-8") as file:
            file.write(plantuml_code)

        subprocess.run(["java", "-Dfile.encoding=UTF-8", "-jar", "plantuml.jar", "-tsvg", output_path])
...
# 外部で指定されたMarkdownファイルと出力PDFファイルのパス
print("Input Markdown file (.md):")
input_markdown_path = input()

print("Output directory for images:")
output_directory = input()

print("Output Markdown file (.md):")
output_md_path = input()

plantuml_matches = extract_plantuml_code_from_markdown(input_markdown_path)
image_paths = generate_diagram_images(plantuml_matches, output_directory)
markdown_content = insert_diagrams_into_markdown(input_markdown_path, image_paths)
